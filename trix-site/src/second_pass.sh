...
echo "== uploads $(date +%T)"; python3 src/compute_media_todo.py "$SCRATCH"; cp "$SCRATCH/media_map.json" media_map.json 2>/dev/null; python3 src/media/upload_media.py "$SCRATCH/images" media_map.json "$SCRATCH/upload_todo.json"
