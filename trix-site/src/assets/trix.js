...
(function () {
  ...
  function seo() {
    ...
    var head = d.head, origin = location.origin, url = origin + location.pathname;
    ...
    if (!head.querySelector('link[rel="canonical"]')) { var c = d.createElement('link'); c.rel = 'canonical'; c.href = url; head.appendChild(c); }
  ...
  function scene(host) {
    ...
    if (kind === 'network' || kind === 'globe') {
      var N = kind === 'globe' ? 420 : 260, pos = new Float32Array(N * 3), vel = [], radius = kind === 'globe' ? 9 : 14;
