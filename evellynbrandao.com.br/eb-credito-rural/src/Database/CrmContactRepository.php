...
class CrmContactRepository extends Db {
	...
	public function query( array $args ) {
		...
		$select_vals   = array();
		if ( $open_statuses ) {
			$open_sql    = '(SELECT COUNT(*) FROM `' . $ts . '` s WHERE s.user_id = c.user_id AND s.deleted_at IS NULL AND s.status IN (' . self::in_placeholders( $open_statuses ) . '))';
			$select_vals = $open_statuses;
		} else {
			$open_sql = '0';
		}
		...
		$orderby       = isset( $args['orderby'] ) && isset( $allowed_order[ $args['orderby'] ] ) ? $allowed_order[ $args['orderby'] ] : 'c.updated_at';
		...
		$sql           = "SELECT c.*, u.display_name AS user_name, u.user_email, {$open_sql} AS open_submissions, (SELECT MAX(a.created_at) FROM `{$ta}` a WHERE a.contact_id = c.id) AS last_activity_at FROM `{$t}` c LEFT JOIN {$wpdb->users} u ON u.ID = c.user_id WHERE {$sql_where} ORDER BY {$orderby} {$order}, c.id DESC LIMIT %d OFFSET %d";
		$count_sql     = "SELECT COUNT(*) FROM `{$t}` c LEFT JOIN {$wpdb->users} u ON u.ID = c.user_id WHERE {$sql_where}";
		$items         = (array) $wpdb->get_results( $wpdb->prepare( $sql, array_merge( $select_vals, $vals, array( $per_page, $offset ) ) ), ARRAY_A ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery,WordPress.DB.PreparedSQL.NotPrepared -- SQL montado com placeholders e listas brancas.
